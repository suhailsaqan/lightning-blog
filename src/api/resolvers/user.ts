...
export default {
  Query: {
    ...
    users: async (parent, args, { models }) => await models.user.findMany(),
    nameAvailable: async (parent, { name }, { models, me }) => {
      ...
      return (
        me.name?.toUpperCase() === name?.toUpperCase() ||
        !(await models.user.findUnique({ where: { name } }))
      );
    },
    topUsers: async (parent, { cursor, within, userType }, { models, me }) => {
      ...
      return {
        cursor:
          users.length === LIMIT ? nextCursorEncoded(decodedCursor) : null,
        users,
      };
    },
  },
  ...
  User: {
    ...
    ncomments: async (user, args, { models }) => {
      return await models.item.count({
        where: { userId: user.id, parentId: { not: null } },
      });
    },
